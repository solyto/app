...
<ContentModal {onClose} title={ts.get.libraries.search} width="2xl" rounded="2xl" p="8">
								...
								{#each results[i]! as result (result.id)}
									...
									{:else}
										<button
											...
											class="flex items-center gap-3 rounded-lg p-2 text-left transition-all hover:bg-c-neutral-1 disabled:opacity-50 dark:hover:bg-s-dark-3 cursor-pointer"
										>
											{#if isImporting}
